...
if (isset($_GET['action']) && $_GET['action'] == 'modifier' && !empty($_GET['id_annonce'])) {
    ...
    $modif_annonce = $modification->fetch(PDO::FETCH_ASSOC);
    $id_annonce = $modif_annonce['id_annonce'];
    $titre_modif = $modif_annonce['titre'];
    $description_courte_modif = $modif_annonce['description_courte'];
    $description_longue_modif = $modif_annonce['description_longue'];
    $prix_modif = $modif_annonce['prix'];
    $categorie = $modif_annonce['categorie_id'];
    $pays_modif = $modif_annonce['pays'];
    $ville_modif = $modif_annonce['ville'];
    $adresse_modif = $modif_annonce['adresse'];
    $cp_modif = $modif_annonce['cp'];
}

if (!empty($_POST['id_annonce'])) {

    $id_annonce = trim($_POST['id_annonce']);
    $titre = trim($_POST['titre_modif']);
    $description_courte = trim($_POST['description_courte_modif']);
    $description_longue = trim($_POST['description_longue_modif']);
    $prix = trim($_POST['prix_modif']);
    $pays = trim($_POST['pays_modif']);
    $ville = trim($_POST['ville_modif']);
    $adresse = trim($_POST['adresse_modif']);
    $cp = trim($_POST['cp_modif']);
    $categorie = trim($_POST['categorie_modif']);

    $enregistrement = $pdo->prepare("UPDATE annonce SET titre = :titre, description_courte = :description_courte, description_longue = :description_longue, prix = :prix, pays = :pays, ville = :ville, adresse = :adresse, cp = :cp, membre_id = :membre_id, categorie_id= :categorie_id WHERE id_annonce = :id_annonce");

    $enregistrement->bindParam(':id_annonce', $id_annonce, PDO::PARAM_STR);
    $enregistrement->bindParam(':titre', $titre, PDO::PARAM_STR);
    $enregistrement->bindParam(':description_courte', $description_courte, PDO::PARAM_STR);
    $enregistrement->bindParam(':description_longue', $description_longue, PDO::PARAM_STR);
    ...
    $enregistrement->bindParam(':membre_id', $_SESSION['membre']['id_membre'], PDO::PARAM_STR);
    $enregistrement->bindParam(':categorie_id', $categorie, PDO::PARAM_STR);
...
<main class="container">
    ...
    <!-- FORMULAIRE MODIFICATION ANNONCE S'AFFICHE ANIQUEMENT SI IL Y A MODIFICATION -->
    ...
    <?php if (isset($_GET['id_annonce'])) {   ?>
        ...
        <form class="row border p-3 bg-grayS shadow p-3 mb-5 rounded" method="post" action="" enctype="multipart/form-data">
            <div class="col-sm-6 text-center text-white">
                <div class="mb-3">
                    <input type="text" class="form-control rounded-pill" id="id_annonce" name="id_annonce" value="<?php echo $_GET['id_annonce']; ?>" hidden>
                </div>
                <div class="mb-3">
                    <label for="titre_modif" class="form-label"><i class="fas fa-pencil-alt seaGreen"></i> Titre</label>
                    <input type="text" class="form-control rounded-pill" id="titre_modif" name="titre_modif" value="<?php echo $titre_modif; ?>">
                </div>
                <div class="mb-3">
                    <label for="description_courte_modif" class="form-label"><i class="fas fa-pencil-alt seaGreen"></i> Description courte</label>
                    <textarea class="form-control rounded-pill" id="description_courte_modif" name="description_courte_modif"> <?php echo $description_courte_modif; ?></textarea>
                </div>
                <div class="mb-3">
                    <label for="description_longue_modif" class="form-label"><i class="fas fa-pencil-alt seaGreen"></i> Description longue</label>
                    <textarea class="form-control rounded-pill" id="description_longue_modif" name="description_longue_modif"><?php echo $description_longue_modif; ?></textarea>
                </div>
                <div class="mb-3">
                    <label for="prix_modif" class="form-label"><i class="fas fa-euro-sign seaGreen"></i> Prix</label>
                    <input type="text" class="form-control rounded-pill" id="prix_modif" name="prix_modif" value="<?php echo $prix_modif; ?>">
                </div>
                <div class="mb-3">
                    <label for="categorie_modif" class="form-label"><i class=" seaGreen fas fa-list-ul"></i> Catégorie</label>
                    <select class="form-control rounded-pill" id="categorie_modif" name="categorie_modif">
            ...
            <div class="col-sm-6 text-center text-white">
                <div class="row">
                    <div class="mb-3 mt-5">
                        <label for="pays_modif" class="form-label"><i class="fas fa-house-user seaGreen"></i> Pays</label>
                        <input type="text" class="form-control rounded-pill" id="pays_modif" name="pays_modif" value="<?php echo $pays_modif; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="adresse_modif" class="form-label"><i class="fas fa-house-user seaGreen"></i> Adresse</label>
                        <input type="text" class="form-control rounded-pill" id="adresse_modif" name="adresse_modif" value="<?php echo $adresse_modif; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="cp_modif" class="form-label"><i class="fas fa-house-user seaGreen"></i> Code postal</label>
                        <input type="text" class="form-control rounded-pill" id="cp_modif" name="cp_modif" value="<?php echo $cp_modif; ?>">
                    </div>
                    <div style="display :none; color:#f55;" id="error-message"></div>
                    <div class="mb-3">
                        <label for="ville_modif" class="form-label"><i class="fas fa-house-user seaGreen"></i> Ville</label>
                        <input type="text" class="form-control rounded-pill" id="ville_modif" name="ville_modif">
                    </div>
                </div>
            </div>
    ...
    <!-- FORMULAIRE AJOUT ANNONCE DISPARAIT SI DEMANDE DE MODIFICATION D'UNE ANNONCE-->
    <?php if (!isset($_GET['id_annonce'])) {   ?>
        <form class="row border p-3 bg-grayS shadow p-3 mb-5 rounded" method="post" action="" enctype="multipart/form-data">
            <div class="col-sm-6 text-center text-white">
